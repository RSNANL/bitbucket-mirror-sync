...
try {
    ...
    $webhooks = @(Get-BitbucketWebhooks -Repository $mirror.bitbucket_repository -Credentials $bitbucketCredentials | Where-Object { $_.description -eq $webhookDescription -and $_.active })
    if ($webhooks.Count -eq 0) { throw 'No active managed Bitbucket push webhook was found.' }
    ...
    Write-Host "  Active webhooks:       $($webhooks.Count)"
